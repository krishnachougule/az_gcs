...
pipeline_options = {
    ...
    'template_location':'gs://cdra_submitted/templates/gcs_bq_followup'
...
def parse_xml(element):
    ...
    def parse_into_df(xml_content):
                    ...
                    for subsection in section.iter('section'):
                        ...
                        for element in section.iter('element'):
                            ...
                            if value is not None:
                                ...
                                value_value = None
                                value_unit = None
                            data.append([xmsnId, ncdrPatientId, followupKey, parentSection_code, parentSection_displayName,childSection_code,childSection_displayName,subsection_Code, Element_code, Element_codeSystem, Element_displayName,  value_code, value_codeSystem, value_displayName, value_type, value_value, value_unit])
                else:
                ...
                for element in section.iter('element'):
                    ...
                    if value is not None:
                        ...
                        if Element_displayName is None:
                            Element_displayName = f"{parentSection_displayName}_{value_type}"
                    data.append([xmsnId, ncdrPatientId, followupKey, parentSection_code, parentSection_displayName,childSection_code,childSection_displayName,subsection_Code, Element_code, Element_codeSystem, Element_displayName,  value_code, value_codeSystem, value_displayName, value_type, value_value, value_unit])        
            for followup in patient.iter('followup'):
                followupKey = followup.get('followupKey')
                for section in followup.iter('section'):            
                    parentSection_code = section.get('code')
                    parentSection_displayName = section.get('displayName')
                    if section.find('section') is not None:
                        counter = 1
        ...
        df = pd.DataFrame(data, columns=['SubmissionId', 'PatientId','followupKey','parentSection_code','parentSection_displayName','childSection_code','childSection_displayName','subsection_Code','Element_code','Element_codeSystem','Element_displayName','value_code','value_codeSystem','value_displayName','value_type','value_value','value_unit'])
        #print(df)
...
def pipeline2():
    ...
    with beam.Pipeline(options=pipeline_options) as p2:
        ...
        WHERE TIMESTAMP_TRUNC(Create_Date_Time, DAY) = TIMESTAMP('{createdDate}') and Case_Type = "F"
        """
